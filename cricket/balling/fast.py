...
def detect_fast_ball(frame, prev_position, ball_color_hsv_range, time_elapsed):
    ...
    if contours:
        ...
        if cv2.contourArea(ball_contour) > 50:
            ...
            if prev_position:
                ...
                if speed > 10:
                    return "Fast Ball", curr_position
            return "Not a Fast Ball", curr_position
...
def analyze_video(video_path:str, ball_color_hsv_range:list[tuple, tuple]):
    ...
    try:
        while cap.isOpened():
            ...
            classification, prev_position = detect_fast_ball(frame, prev_position, ball_color_hsv_range, time_elapsed)
...
def classify_entire_video(video_path:str, ball_color_hsv_range:list[tuple, tuple]):
    ...
    fast_count = sum(1 for v in analysis.values() if v == "Fast Ball")
    not_fast_count = sum(1 for v in analysis.values() if v == "Not a Fast Ball")
    total_frames = len(analysis)

    if total_frames == 0:
        return "No Valid Frames Detected"

    if fast_count / total_frames >= 0.5:
        return "Good Fast Ball"
    elif not_fast_count / total_frames >= 0.5:
        return "Bad Fast Ball"
    else:
        return "Not a Fast Ball"
